...
__global__ void ssslu_forward_kernel(const float* x, float* output, int size) {
    ...
    if (base + 3 < size) {
        ...
        // elementwise ssslu computation
        float o0 = e0 * ((e0 * __frcp_rn(1.0f + fabsf(e0))) * 0.5f + 0.5f);
        float o1 = e1 * ((e1 * __frcp_rn(1.0f + fabsf(e1))) * 0.5f + 0.5f);
        float o2 = e2 * ((e2 * __frcp_rn(1.0f + fabsf(e2))) * 0.5f + 0.5f);
        float o3 = e3 * ((e3 * __frcp_rn(1.0f + fabsf(e3))) * 0.5f + 0.5f);
...
__global__ void ssslu_backward_kernel(const float* x, const float* grad_out, float* grad_x, int size) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (base + 3 < size) {
        ...
        float t0 = e0 + 1;
        float o0 = ((e0 + 2) * fabsf(e0) + (t0 * t0)) * ((inv0 * inv0) * 0.5f);
        float inv1 = __frcp_rn(1.0f + fabsf(e1));
        float t1 = e1 + 1;
        float o1 = ((e1 + 2) * fabsf(e1) + (t1 * t1)) * ((inv1 * inv1) * 0.5f);
        float inv2 = __frcp_rn(1.0f + fabsf(e2));
        float t2 = e2 + 1;
        float o2 = ((e2 + 2) * fabsf(e2) + (t2 * t2)) * ((inv2 * inv2) * 0.5f);
        float inv3 = __frcp_rn(1.0f + fabsf(e3));
        float t3 = e3 + 1;
        float o3 = ((e3 + 2) * fabsf(e3) + (t3 * t3)) * ((inv3 * inv3) * 0.5f);
...
at::Tensor ssslu_forward_cuda(const at::Tensor& x) {
    ...
    ssslu_forward_kernel<<<numBlocks, blockSize>>>(
...
at::Tensor ssslu_backward_cuda(const at::Tensor& x, const at::Tensor& grad_output) {
    ...
    ssslu_backward_kernel<<<numBlocks, blockSize>>>(
        x.data_ptr<float>(),
